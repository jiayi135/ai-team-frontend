...
export default function Costs() {
  ...
  const fetchCostData = async () => {
    try {
      ...
      // 检查响应状态
      const responses = [summaryRes, providerRes, categoryRes, historyRes, alertsRes];
      ...
      console.error('Fetch cost data error:', err);
    ...
    >
      {isLoading && !summary ? (
        ...
        <>
          {/* Error Message */}
          {error && (
            <div className="bg-red-50 border border-red-100 rounded-xl p-4 text-red-700 text-sm mb-6 flex justify-between items-center">
              <span>{error}</span>
              <button onClick={fetchCostData} className="px-3 py-1 bg-red-100 hover:bg-red-200 rounded-lg transition-colors">重试</button>
            </div>
          )}
            ...
            <div className="bg-white rounded-lg border border-slate-200 p-6 shadow-sm">
              ...
              <div className="space-y-5">
                {providerCosts.map((item) => (
                  <div key={`provider-${item.provider}`}>
            ...
            <div className="bg-red-50 border border-red-100 rounded-lg p-6">
              ...
              <div className="space-y-3">
                {alerts.map((alert, idx) => (
                  <div key={`alert-${idx}`} className="flex items-start gap-3 p-3 bg-white rounded border border-red-100">
